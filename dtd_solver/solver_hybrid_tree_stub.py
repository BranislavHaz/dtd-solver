...
def _score_sheet(sh: SheetResult) -> Tuple[int, int, int, int]:
    ...
    # PRIORITY: maximize placed area, then maximize placed count, then minimize waste, then minimize cut
    return (-placed_area, -placed_count, waste, cut)
...
def _choose_best_sheet_twolevel(
    ...
    # Mild sampling for speed (keep top 60% of candidates)
    if len(cand_x1) > 8:
        step = max(1, len(cand_x1) // 8)
        cand_x1 = cand_x1[::step]
    if len(cand_y1) > 8:
        step = max(1, len(cand_y1) // 8)
        cand_y1 = cand_y1[::step]
